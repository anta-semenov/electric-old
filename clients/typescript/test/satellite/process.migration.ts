...
import { ContextType as CommonContextType, relations } from './common'
...
export const commonSetup = async (t: ExecutionContext<ContextType>) => {
...
const populateDB = async (t: ExecutionContext<ContextType>) => {
...
export const processMigrationTests = (test: TestFn<ContextType>) => {
  ...
  test.serial(
    ...
    async (t) => {
      ...
      */
      ...
      const { satellite, adapter, txDate, getMatchingShadowEntries } = t.context
      const timestamp = txDate.getTime()
  ...
  test.serial(
    ...
    async (t) => {
      ...
      const rowsBeforeMigration = await fetchParentRows(adapter)
      const rowsBeforeMigrationExceptConflictingRow =
        rowsBeforeMigration.filter((r) => r.id !== deleteRow.id)
      ...
      const conflictingRow = rowsAfterMigration.find(
        (r) => r.id === deleteRow.id
      )
